...
exp_colname="$(echo -e "chr\tsnp\tstart\tend\tp_lrt")"
...
output_file="processing_data/tmp_new.tsv"
 ...
 cut -f1-4 processing_data/tmp_new.tsv > processing_data/tmp1.tsv 
 ...
 rm -r tmp_new.tsv tmp_file.tsv tmp1.tsv combined_snp.tsv tmp.tsv

else
 echo "Error: Make sure columns are in proper order (chr, snp, start, end, p_lrt)"
fi
