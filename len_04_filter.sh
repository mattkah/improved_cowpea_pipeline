...
exp_colname="$(echo -e "chr\tstart\tend\tsnp")"
...
then
 ...
 while IFS=$'\t' read -r gene_id chr gene start end homolog_id homolog_type gene_name species gene_desc protein_coding; do
    ...
    while IFS=$'\t' read -r chr_s start_s end_s snp_s; do
     ...
     done < processing_data/tmp_snp_file.tsv
 ...
 rm -r tmp_snp_file.tsv combined_snp.tsv tmp.tsv
	
else
 echo "Make sure columns are in proper order (chr, start, end, snp)"
fi
